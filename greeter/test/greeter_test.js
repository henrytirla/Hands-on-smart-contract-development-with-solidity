...
contract("Greeter", (accounts) => {
    ...
    describe("greet()", () => {
        it("returns 'Hello, World!'", async () => {
            const greeter = await GreeterContract.deployed();
            const expected = "Hello, Tirla";
            const actual = await greeter.greet();

            assert.equal(actual, expected, "greeted with 'Hello, Tirla'");
        })
    })
...
contract("Greeter: update greeting", (accounts) => {
    describe("setGreeting(string)", () => {
        describe("when message is send by the owner", () => {
    ...
    describe("when message is send by another account", () => {
        it("does not set the greeting", async () => {
            ...
            try {
                ...
                assert.equal(err.reason, errorMessage, "greetings should not update");
                return;
            }
